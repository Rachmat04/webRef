/**
 * WebRef – Wikipedia Citation Generator
 * Version: 2021-09-01
 * Source:  https://en.wikipedia.org/wiki/User:V111P/js/WebRef
 * Help:    https://id.wikipedia.org/wiki/Pengguna:Rachmat04/webRef.js
 *
 * This script runs in a browser on any web page. It inspects the page's
 * metadata – Open Graph tags, schema.org microdata, standard meta elements,
 * and page structure – to extract the article title, publication date,
 * author names, site name, content language, and canonical URL. It then
 * formats these details into a Wikipedia {{Cite web}} template and displays
 * the result in a floating toolbar fixed to the top of the page.
 *
 * Key features:
 *  - Automatic extraction of title, date, author, publisher, and language
 *  - Date formatting in DMY, MDY, or YMD styles
 *  - Author name parsing and formatting (last / first)
 *  - Site-specific configuration saved to browser local storage
 *  - One-click copy of the completed citation to the clipboard
 *  - Localised for Indonesian Wikipedia (id.wikipedia.org)
 *
 * Localisation: search for "// LOCALIZE" to find every setting that must
 * be adjusted when deploying this script on a different wiki or language.
 */
window.webRef = window.webRef || {}; // Object used to communicate with webRefSetup.
window.webRef.getRef = (function () {
  "use strict";

  var DEBUG = false; // Set to true to enable debug logging in the browser console

  function prt(txt) {
    if (DEBUG && window.console && console.log) console.log(txt);
  }

  window.webRef.webRefVer = 1;

  var webRefSetupJsUrl =
    "//en.wikipedia.org/w/index.php?title=" +
    "User:V111P/js/webRefSetup.js&action=raw&ctype=text/javascript&smaxage=0&maxage=0";
  var helpUrl = "//id.wikipedia.org/wiki/Pengguna:Rachmat04/webRef.js"; // LOCALIZE

  // Month name abbreviations used by the target websites. Names must be in lower case.
  // January = 0, December = 11.
  var monthNameToNum = {
    // LOCALIZE: add abbreviations for the local language.
    jan: 0,
    feb: 1,
    mar: 2,
    apr: 3,
  ...
  // Month names used in the formatted output.
  var monthNumToName = [
    // LOCALIZE: translate month names.
    "Januari",
    "Februari",
    "Maret",
    "April",
  ...
  // Words used when splitting names where multiple authors are listed.
  var andWords = ["dan"]; // LOCALIZE: add the word(s) for "and" in the local language.

  // Date format tokens: %D% or %DD%, %M% or %MM% or %MMM%, %YY% or %YYYY%.
  // %M% → Feb = 2; %MM% → Feb = 02; %MMM% → Feb = monthNumToName[1].
  ...
  var templateParams = {
    ...
    title: "title",
    transTitle: "trans-title", // Title translated to Indonesian.
    work: "work",
    date: "date",
    ...
    barV: "| ", // Bar and spaces (without newlines) for vertical format.
    barVsameLine: " | ", // First name on the same line as last name in vertical mode.
    barH: " |", // Bar and spaces for horizontal format.
    eqV: " = ", // Equal sign and spaces (without newline) for vertical format.
    eqH: "=", // Equal sign for horizontal format.
    urlEqH: "= ", // A space before the URL aids line wrapping.
  };
  ...
  var msgs =
    (webRef.idToText =
    webRef.msgs =
      webRef.msgs || webRef.idToText || {});
  var msgs_default = {
    // LOCALIZE
    programName: "WebRef",
    monthWarning: "Check month!",
    monthWarningTitle:
      "Was the date in the MM/DD/YY(YY) or DD/MM/YY(YY) format?",
    quoteUsedWarning: "Quotation!",
    quoteUsedWarningTitle:
      "The selected text on the page was used for the value of the Quote parameter",
    ...
    formatNamesPrompt:
      "Enter the author names (separated by commas if more than one author) to be formatted and inserted into the template:",
    dmy: "DMY",
    mdy: "MDY",
    ymd: "YMD",
    refName: "RefName",
    ...
    authorName_L:
      "For personal websites, formatted author name (e.g. Smith, John):",
    authorWikiArticle_L:
      "For personal websites, wiki article about the author:",
    notAnAuthor_L:
      "This is not an author's name (this at the place where the author's name usually is, means the article is anonymous):",

    buttonsIntro:
      'First click the first button. The code corresponding to the information from the form above will appear in the text box below. Now you can press the button "Use the code..." to see if the result is good. After that you can come back here with the button "Site setup". Then you can save the code in the "local storage" (see Web storage in Wikipedia), where you can use it on every page on this domain/subdomain. It is recommended that you also save it elsewhere so that you can restore it easily if it gets deleted from the local storage.',
    toCode:
      "Create the code! Convert the information in the form above into code in the text box",
    saveToStorage: "Save the code from the text box - into the local storage",
    loadFromVar:
      "Show in the text box the currently-active settings for this site",
    ...
    delStorageConfirm:
      "Delete the settings for this site from the local storage?",
    overwriteStorageConfirm:
      "Saving the new settings into the local storage will overwrite the old settings for this site.",
    invalidCode: "The code in the text box is not valid.",
    noStorage:
      "Your browser does not support Web Storage and/or JSON or for some reason they don't work on this site.",
  ...
  var langToIgnore = "id"; // LOCALIZE: the wiki's default language — omitted from the cite template.
  var selectionJoiner = " [...] "; // LOCALIZE: string used to join multiple text selections.
  ...
  var metaContent;
  var siteObj; // Populated from webRefSiteData or local storage; see go().
  var refFrame = {
    frame: null,
    win: null,
    doc: null,
  ...
  var codeTextArea;
  var siteLang; // Used in date formatting when equal to 'en-US' and the month is numeric.
  var monthWarningNeeded; // Set by dateFormatter() when the date format (MM/DD vs DD/MM) is ambiguous.
  ...
  var refDocData = (window.webRef.refDocData = {
    things: {
      ...
      siteName: {
        // The site domain is also used as a fallback.
        autoSearchIn: [
          "meta-p og:site_name",
          "meta parsely-site",
      ...
      lang: {
        formatter: function (langCode) {
          // Strips the language variant suffix, e.g. 'en-GB' → 'en'.
  ...
  var dom = (window.webRef.dom = {
      ...
      function getText(node) {
        ...
        if ((node = node.firstChild))
          do {
            ...
            if (nn == "br") txt += " ";
            else if (nn != "script" && nn != "style") txt += getText(node);
          } while ((node = node.nextSibling));

        // The textContent property does not replace <br> with a space, hence manual traversal.
        return txt;
      }
    ...
    newEl: function (tagName, attrs) {
      ...
      if (attrs)
        for (var a in attrs) {
          if (a == "css") dom.setStyle(el, attrs[a]);
          else if (a == "text") {
            // The document.all property is a legacy IE check; el.textContent is sufficient for modern browsers
            if (document.all) el.innerText = attrs[a];
            else el.textContent = attrs[a];
          } else
            try {
              el[a] = attrs[a];
            } catch (e) {
              aux.fatalError("newEl: el=" + el + ", a=" + a + ", err=" + e);
            }
        }
      return el;
    },
  ...
  function getMetaContent() {
    ...
    for (var i = 0, el, prop; i < metaEls.length; i++) {
      ...
      prop = el.getAttribute("itemprop"); // schema.org microdata (itemprop attribute).
      if (prop) metaContent["meta-i " + prop] = el.content;

      if (el.name) metaContent["meta " + el.name] = el.content;
      else {
        prop = el.getAttribute("property");
        if (prop) metaContent["meta-p " + prop] = el.content;
      }
    }
  ...
  // Pre-computed alternation string for month-name matching (e.g. 'jan|feb|...').
  var monthRegexStr = (function () {
    var parts = [];
    for (var m in monthNameToNum) {
      if (monthNameToNum.hasOwnProperty(m)) parts.push(m);
    }
    return parts.join("|");
  })();

  function dateFormatter(dateStr, format, useUSFormatIfUnclear) {
    // The format parameter is optional.
    var formattedDate = "";
    format = format || dateFormatDefault;

    var monthStr = monthRegexStr;
    var month;
      ...
      if (dateParts) {
        if (dateParts[1].length == 4) {
          ...
          if (year < thisYearMod100 + 1)
            year = "20" + year; // 2000s: last two digits are less than those of the current year.
          else if (year < 100) year = "19" + year; // 1900s otherwise.

          if (siteLang == "en-US" || dateParts[2] > 12) {
            // American-style date (month first).
            month = dateParts[1];
            day = dateParts[2];
          } else {
            if (!siteLang || siteLang == "en") monthWarningNeeded = true;
  ...
  function formatDateParams(format, useUSFormatIfUnclear) {
    ...
    function formatDateParam(templStr, format, paramName) {
      ...
      var dateInTemplate = trimCollapse(
        (templStr.match(dateParamRe) || ["", ""])[1],
      );
  ...
  function formatNamesPrompt() {
    ...
    for (var j = 0; j <= 9; j++)
      names +=
        (val.match(strt + tp.first + (j == 0 ? "" : j) + end) || ["", ""])[1] +
        " " +
        aux
          .collapseWhitespace(
            (val.match(strt + tp.last + (j == 0 ? "" : j) + end) || [
              "",
              "",
            ])[1],
          )
    ...
    if (userAuthors) {
      ...
      if (namesNewParams) {
        var pos = val.search(
          new RegExp("\\|\\s*(" + tp.last + "1?|" + tp.author + ")\\s*="),
        );
        if (pos < 0) pos = val.indexOf("|");
        if (pos < 0) pos = 0;
        for (var k = 0; k < authorParamNames.length; k++)
          val = val.replace(new RegExp(strt + authorParamNames[k] + end), "");
  ...
  function nameFormatter(nameStr) {
    var result = [[""]];
    // Replace all 'and' conjunctions with commas so that individual names can be parsed.
    // For example, "G. D., B. R., and A. S." becomes: [ ['D.', 'G.'], ['R.', 'B.'], ['S.', 'A.'] ].
    nameStr = nameStr
      .replace(new RegExp("(,?\\s+)(" + andWords.join("|") + ")\\s", "gi"), ",")
      .replace(/\s+/g, " ")
      .replace(/ ?, ?/g, ",")
      .replace(/,,+/g, ",");
    ...
    for (var i = 0; i < peopleArr.length; i++) {
      ...
      if (lastSpacePos > 0) result[i][1] = person.slice(0, lastSpacePos);
    }
    return result;
  }
  ...
  function authorParams(authorsArr, additionalNotAnAuthor) {
    ...
    if (!authorsArr && siteObj.authorName) {
      ...
      (authorsArr = authorsArr || searchFor("author", siteObj.notAnAuthor)) !==
      null
    ) {
      if (additionalNotAnAuthor) {
        // Remove appended organisation names, e.g. "CNN" from "James Smith, CNN".
      ...
      if (authorsArr.length == 1 && !authorsArr[0][1])
        // Single author without a first name: use |author=.
        str = nl + tp.author + eqV + authorsArr[0][0];
      else {
        // Multiple authors or author with first name: use |last= and |first=.
        for (var i = 0; i < authorsArr.length; i++) {
          var num = i > 0 || authorsArr.length > 1 ? String(i + 1) : ""; // Omit the number suffix when there is only one author.
          if (authorsArr[i][0]) {
  ...
  function singleMultiLine(toMulti) {
    ...
    if (toMulti === false || (!toMulti && nowMulti)) {
      val = val
        ...
        // A space after |url= aids line wrapping.
        .replace(
          new RegExp("(\\|\\s*" + tp.url + ")\\s*=\\s*"),
          "$1" + tp.urlEqH,
        );
    } else {
      val = val
        ...
        .replace(/\s*}}/, "\n}}")
        .replace(
          // Place the |last= and |first= parameters on the same line.
          new RegExp(
            "(\\|\\s*" +
              tp.last +
              "\\d?\\s*=[^\\n|}]*)\n(\\| " +
  ...
  function setup() {
    if (webRef.webRefSetup) webRef.webRefSetup();
    else {
  ...
  function createUI() {
    ...
    var buttons = [
      {
        ...
        onclick: function () {
          ...
          val = val
            .replace(
              new RegExp("\\s*\\|\\s*" + tp.quote + "\\s*=\\s*(\\||})"),
              "$1",
            ) // Remove empty |quote= parameter.
            .replace(
              new RegExp("\\s*\\|\\s*" + tp.lang + "\\s*=\\s*(\\||})"),
              "$1",
            ) // Remove empty |language= parameter.
            .replace(/\s+}}\s*$/, "}}"); // Remove whitespace before the closing }}.
          codeTextArea.value = val;
          singleMultiLine(false);
          codeTextArea.focus();
          codeTextArea.select();
          if (copyButtonSupported) {
            // document.execCommand('copy') is deprecated; consider migrating to
            // navigator.clipboard.writeText() once cross-origin iframe restrictions allow.
      ...
      },
      {
        id: "dmy",
        onclick: function () {
          formatDateParams(dateFormatDMY);
        },
      },
      {
        id: "ymd",
        onclick: function () {
          formatDateParams(dateFormatYMD);
        },
      },
      {
        id: "refName",
    ...
    if (refFrame.frame) document.body.removeChild(refFrame.frame);
    subDiv = dom.byId("ref00refDiv");
    if (subDiv) document.body.removeChild(subDiv);
  ...
  function elFromStr(selector) {
    ...
    function findSubEl(parent, i) {
      if (i >= elSelParts.length) return parent;
      var s = elSelParts[i];
      var el = null;
      if (s.charAt(0) == "#") {
        ...
        if (el === null) {
          prt(
            "WebRef debug info: No element with id " +
              s.slice(1) +
              " found in document.",
          );
          return null;
        } else return findSubEl(el, i + 1);
      } else {
        var n = parseInt(s, 10) || 0;
        if (n) s = s.slice((n + "").length);
        var classes = s.split(".");
        var tagName = classes[0];
        classes = classes.slice(1);
        var els = parent.getElementsByTagName(tagName.toUpperCase());
        el = null;
        if (classes.length > 0) el = parent.querySelector(s);
        else el = els[n];
        if (el === null) {
          prt(
            "WebRef debug info: No element with id " +
  ...
  // Returns the text content of the element at the given address.
  // If pre- and post-position strings are included in the address, returns only the text between them.
  // Whitespace is collapsed to a single space.
  // The element's background colour is set to yellow unless highlight is explicitly false.
  var textFromAddr = (window.webRef.textFromAddr = function (
    elAddr,
    highlight,
  ) {
    if (!elAddr) return "";
    ...
    if (
      selector.slice(0, 4) == "meta" ||
      selector.slice(0, 4) == "misc" ||
      selector == "title"
    ) {
      ...
      if (el) {
        text = dom.getText(el);
        if (highlight !== false) el.style.backgroundColor = "yellow";
      }
    }
    ...
    if (text && parts.length > 1) {
      ...
      var matched = text.match(
        parts[1] + "\\s*(.+" + (!ending ? ")" : "?)\\s*" + ending),
      );
      if (matched && matched[1]) text = matched[1];
    }

    return aux.collapseWhitespace(text);
  });

  // Searches the autoSearchIn list for thingName and returns the first match found.
  ...
  function dateToString(day, month, year, format) {
    // Month is 0–11.
    day = +day;
    month = +month;
    year = +year;
  ...
  function makeRefName(title) {
    ...
    const timestamp = hh + mm + ss; // Formatted as HHmmss.
  ...
  // Returns the text for thingName, checking siteObj first then falling back to autoSearchIn.
  // Returns null if the result matches the exception string.
  // Applies the formatter from refDocData.things[thingName].formatter, if defined.
  function searchFor(thingName, exception) {
    var text = textFromAddr(siteObj[thingName]) || autoSearchFor(thingName);
    if (text == exception) return null;
    var formatter = refDocData.things[thingName].formatter;
    if (formatter) text = formatter(text);
    return text;
  }
  ...
  function go() {
    ...
    siteObj =
      (window.webRefSiteData && webRefSiteData[domain]) ||
      (function () {
        var str, sO;
    ...
    date = searchFor("date");
    if (monthWarningNeeded) {
      // Set by dateFormatter() when the date format is ambiguous.
      warnings.push([msgs.monthWarning, msgs.monthWarningTitle]);
    }

    langParam = "";
    if (siteLang.search(/^id-[Ii][Dd]/i) == 0) siteLang = "id-[Ii][Dd]";
    lang = siteLang == "id-[Ii][Dd]" ? "id" : siteLang;
    if (!lang) {
      lang = autoSearchFor("lang") || "";
      var langFormatter = refDocData.things.lang.formatter;
      if (lang && langFormatter) lang = langFormatter(lang);
    }
    if (!lang || lang != langToIgnore)
      // Add the language parameter only if it differs from langToIgnore.
    ...
    codeTextArea.value =
      ...
      eqV +
      title +
      (!lang || lang == langToIgnore ? "" : "\n" + barV + tp.transTitle + eqV) + // Placeholder for the translated title; included only for non-default languages.
